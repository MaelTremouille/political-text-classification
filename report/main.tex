...
The main result is that TF-IDF with logistic regression reaches 84\% accuracy even after removing all explicit party names from the text. CamemBERT, despite being a much larger model, only gets to 71\%. We initially expected CamemBERT to be better, because it can capture contextual information that TF-IDF ignores. But in practice, the discriminative signal in \textit{professions de foi} seems to be mostly about which specific words are used, not about how they relate to each other in context. A simple bag-of-words is enough to pick up on that. It may also be because of the length of 512 tokens which is not sufficient.
...
A natural extension would be to improve the labeling of right-wing parties by matching candidates against electoral result databases. It would also be interesting to apply the same approach to more recent elections, where party discourse may have diverged more.
