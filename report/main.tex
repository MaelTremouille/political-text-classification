...
Using text to analyze political positions has been explored quite a lot. \citet{laver2003extracting} proposed Wordscores, a method that scores words based on reference texts with known political positions, and then uses these scores to place new texts on a left-right scale. They tested it on party manifestos in Britain and Ireland. The Manifesto Project~\citep{volkens2013manifesto} built a large hand-coded database of party platforms across many countries, which is widely used in political science. \citet{rheault2020word} did something different: they trained word embeddings with party metadata on parliamentary debates from Britain, Canada, and the US, and showed that it captures ideological differences.

What we do is somewhat related to Laver et al., but instead of placing parties on a continuous scale, we classify documents into discrete families. We also work with French texts rather than English ones, and we compare TF-IDF with a pretrained transformer, which none of the above papers do.

\paragraph{NLP for French political texts.}
Most of this literature focuses on English. For French, there is not much. The Archelec project~\citep{archelec} made a large corpus of \textit{professions de foi} available, but their work is about digitization and OCR, not about NLP tasks like classification.

\paragraph{CamemBERT.}
CamemBERT~\citep{martin2020camembert} is a RoBERTa-based model pretrained on French web data (OSCAR). It works well on standard French NLP benchmarks like NER, POS tagging, and text classification, so it seemed like a good fit for our task.
...
We then switched to a sentence-transformer\footnote{\texttt{dangvantuan/sentence-camembert-large}, available at \url{https://huggingface.co/dangvantuan/sentence-camembert-large}}, which is built on CamemBERT-large and fine-tuned for semantic similarity. We use the large version because this is the only one available as a sentence-transformer on Hugging Face, and since we do not fine-tune it, the size is not a problem. It uses mean pooling over all token embeddings instead of just the \texttt{[CLS]} token, which gave us much more usable results.
