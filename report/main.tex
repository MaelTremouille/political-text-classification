...
To understand what the model actually learns, we extracted the top logistic regression coefficients per class --- that is, the TF-IDF features that contribute the most to predicting each family. The results are quite revealing.

For \textit{Extr\^{e}me gauche}, the most discriminative word is ``travailleurs'' (\textit{workers}), followed by ``les travailleurs''. This is the typical vocabulary of LO and LCR manifestos, which consistently use class-struggle language. We also observe that many function words (``les'', ``de'', ``des'') appear with high coefficients for this class, which we think is partly an artifact of text length: since these documents are on average 6 times longer than Droite texts (Table~\ref{tab:lengths}), common French words end up with higher TF-IDF scores simply because they accumulate more.
...
The feature analysis from Section~\ref{sec:results} helps us understand this. The classifier relies heavily on a few distinctive keywords that are specific to each political family. Extr\^{e}me gauche texts use class-struggle vocabulary (``travailleurs'', ``lutte'') that almost never appears in other families, which explains the very high F1 of 0.96. This is consistent with what \citet{laver2003extracting} found with Wordscores: word frequencies alone can capture political positioning, even without understanding context or syntax. Our results show that this holds for French \textit{professions de foi} as well, with a simple logistic regression.

However, the feature analysis also revealed some issues with our approach. The presence of ``Le~Pen'' as a top feature for Extr\^{e}me droite, and of ``communiste'' and ``gauche'' for Gauche, means the model is partially relying on markers that our cleaning should have removed (or at least, that we should have thought about removing). The Extr\^{e}me droite performance (F1 = 0.76) may therefore be somewhat inflated. A more thorough cleaning --- removing not just party names but also leader names and generic ideological labels --- would give a fairer picture.

\paragraph{The Gauche--Droite confusion.}
The confusion matrix shows that the main classification error is between Gauche and Droite: 76 Gauche texts are predicted as Droite, while 12 Droite texts are predicted as Gauche. This is actually an interesting result from a political science perspective. During the 1973--1993 period, France went through the \textit{alternance} of 1981 (first left-wing president under the Fifth Republic) and then several \textit{cohabitation} periods where the president and prime minister came from opposite sides. These events pushed both the PS and the mainstream right toward more similar policy language --- both sides ended up talking about the same issues (employment, housing, modernization) in similar terms. The fact that a classifier struggles to tell them apart once party names are removed is, in a way, evidence of this convergence.

The cosine similarity heatmap (Figure~\ref{fig:cosine}) paints a similar picture. Most parties end up with similarity above 0.90, meaning their discourse is actually quite close once you look at the embeddings. The RPCR is the main exception, which makes sense since it is a New Caledonian party with very different concerns from metropolitan politics. We also notice that LO and PSU are slightly less similar to right-wing parties, which is coherent with their more radical positioning --- and with the classification results showing that the extremes are easier to identify.

Regarding the embedding approach, our first experiment with CamemBERT base \texttt{[CLS]} tokens was a failure --- all cosine similarities were above 0.98, which told us nothing (see Appendix~\ref{app:cls_failure}). The sentence-transformer gave much more usable results, but this required knowing that a base model trained for masked language modeling is not the right tool for computing sentence-level similarity.

\paragraph{Why CamemBERT underperforms.}
We think there are several reasons, not just one. First, the dataset is probably too small: 3{,}163 training examples for 110M parameters leads to overfitting, which we clearly see in the training curves. But it is not just a data problem. The discriminative signal in these texts is mostly lexical --- specific keywords that TF-IDF captures perfectly well --- so the contextual representations that CamemBERT learns do not bring much additional value. The 512-token limit is also very penalizing here, especially for Extr\^{e}me gauche texts that average 6{,}700 words: the model only sees the first 5--6\% of these documents. TF-IDF does not have this limitation. In the end, CamemBERT's underperformance is not a failure of the model itself, but rather tells us something about the nature of the classification signal in this corpus: it is a keyword-level signal, not a contextual one.
...
Second, the feature analysis showed that our text cleaning was not complete. Words like ``communiste'', ``gauche'', and candidate names like ``Le~Pen'' remain in the cleaned text and contribute to classification. A stricter cleaning (removing individual political words, not just full party names) would be needed to truly test whether the model can classify based on discourse content alone.

Third, the OCR quality varies a lot across election years. Documents from 1973 and 1978 have noticeably more errors than later ones, and we did not apply any correction. This adds noise to both feature extraction and classification, though it is hard to say how much it actually affects the results.

Fourth, CamemBERT only processes the first 512 tokens, which means that for longer documents, the model misses most of the text. We considered chunking or hierarchical approaches but decided against it given the added complexity. The t-SNE plots should also be interpreted carefully: t-SNE can distort global distances, and the temporal plot (Figure~\ref{fig:temporal}) only has 18 points, which is really not a lot.

Finally, the large difference in text length between families (Table~\ref{tab:lengths}) means that TF-IDF features partly capture length rather than content. A document with 6{,}000 words will naturally accumulate higher TF-IDF scores for common words than a document with 1{,}000 words. We did not normalize for length, and this could explain part of the high performance on Extr\^{e}me gauche.
...
Several extensions would be natural. First, a more thorough text cleaning --- removing not just party names but also leader names and generic political labels --- would give a fairer evaluation of whether the model truly learns from discourse content. Second, matching candidates against electoral result databases could improve the labeling of right-wing parties and reduce class imbalance. Finally, it would be interesting to apply the same approach to more recent elections, where party discourse may have diverged more, especially with the rise of new political movements.
